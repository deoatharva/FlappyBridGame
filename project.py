...
def flappygame():
	...
	while True:
		...
		for pipe in up_pipes:
			pipeMidPos = pipe['x'] + game_images['piped'][0].get_width()/2
		...
		if up_pipes[0]['x'] < -game_images['piped'][0].get_width():
			up_pipes.pop(0)
			down_pipes.pop(0)
		window.blit(game_images['background'], (0, 0))
		for upperPipe, lowerPipe in zip(up_pipes, down_pipes):
			window.blit(game_images['piped'][0],
						(upperPipe['x'], upperPipe['y']))
			window.blit(game_images['piped'][1],
...
def isGameOver(horizontal, vertical, up_pipes, down_pipes):
	if vertical > elevation - 25 or vertical < 0:
		return True
	for pipe in up_pipes:
		pipeHeight = game_images['piped'][0].get_height()
		if(vertical < pipeHeight + pipe['y'] and\
		abs(horizontal - pipe['x']) < game_images['piped'][0].get_width()):
			return True
...
if __name__ == "__main__":
	...
	game_images['flappybird'] = pygame.image.load(
		bplayer).convert_alpha()
	game_images['sea_level'] = pygame.image.load(
		sl).convert_alpha()
	game_images['background'] = pygame.image.load(
		bg).convert_alpha()
	game_images['piped'] = (pygame.transform.rotate(pygame.image.load(
		piped).convert_alpha(), 180), pygame.image.load(
	piped).convert_alpha())
